...
//----
   ...
   for(i=0, y15m=0,  y4h=0,  y1h=0,  y30m=0;i<limit;i++)
   {
      ...
      for (int tf = 0; tf < 4; tf++)
      {
         ...
         switch (tf)
         {
            case 0: prd = Period_1; cci_n = cci_1; yy = y15m;   break;
            case 1: prd = Period_2; cci_n = cci_2; yy = y30m;   break;
         ...
         {
            string sDir = "";
            if (buf1_up[0] + buf2_up[0] + buf3_up[0] + buf4_up[0] >= 4+Gap*6-0.001)
               sDir = "Up";
            if (buf1_down[0] + buf2_down[0] + buf3_down[0] + buf4_down[0] >= 4+Gap*6-0.001)
               sDir = "Down";
            if (sDir != "")
            {
               //PlaySound("alert1.wav");
               Alert("Forex freeway - Direction ",sDir);
            }
         }         
